...
if (isset($_POST['xAccion2'])) {
    if ($_POST["xAccion2"] == "upload") {
        ...
        $cve_calendario = isset($_POST['xCveCalendario']) ? $_POST['xCveCalendario'] : 0;
        $target_dir = "../img/calendario_actividades/";
        ...
        $new_name_file = $cve_calendario . $extension;
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                ...
                $sql = "UPDATE calendario_actividades SET imagen_portada = '" . (substr($target_file, 3, strlen($target_file))) . "' WHERE cve_calendario = $cve_calendario";
        ...
        header('Location:cat_calendario_actividades.php');
        return;
    }
}
?>
<div class="modal-header">
    ...
    <h4 class="modal-title">Calendario actividades | Subir imagen portada</h4>
</div>
    ...
    <div class="te">
        <form role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" id="frmUpload" name="frmUpload">
            <div class="form-group">
                <input type="hidden" id="xCveCalendario" name="xCveCalendario" value="<?php echo($_GET["xCveCalendario"]); ?>" />
